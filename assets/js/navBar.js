...
const createThumb = (pages) => {
    ...
    if (!$('.container-thumbs').is(":visible")) {
        ...
        for (let i = 1; i <= pages; i++) {
            ...
            if (i == 1 || i == pages) {
                listThumbs.append($('<div/>', { id: 'page-thumb', class: 'page-thumb single-page options' }).append($('<p/>', { class: 'options' }).html("" + i), $('<div/>', { class: 'page-img options', 'onclick': 'goPage(' + i + ')' }).append($('<img/>', { class: 'options' + i, src: './assets/pages-es/' + i + '.jpg' }))))
            } else if (i % 2 == 0) {
                listThumbs.append($('<div/>', { id: 'page-thumb', class: 'page-thumb left-page options' }).append($('<p/>', { class: 'options' }).html("" + i), $('<div/>', { class: 'page-img options', 'onclick': 'goPage(' + i + ')' }).append($('<img/>', { class: 'options' + i, src: './assets/pages-es/' + i + '.jpg' }))))
            } else {
                listThumbs.append($('<div/>', { id: 'page-thumb', class: 'page-thumb right-page options' }).append($('<p/>', { class: 'options' }).html("" + i), $('<div/>', { class: 'page-img options', 'onclick': 'goPage(' + i + ')' }).append($('<img/>', { class: 'options' + i, src: './assets/pages-es/' + i + '.jpg' }))))
            }
        }
