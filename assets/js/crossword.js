...
(function($) {
    $.fn.crossword = function(entryData) {
        ...
        */
        ...
        this.after('<div id="puzzle-clues"><div class="textClaves"><ol id="across"></ol><ol id="down"></ol></div></div>');
        this.after('<div id="puzzle-btn"><button id="solution">Resolver juego</button><button id="reset">Reiniciar juego<i class="bx bx-refresh" style="color:#ffffff; font-size:20px"></i></button></div>');
        ...
        var puzInit = {
            ...
            init: function() {
                ...
                //Solucion puzzle
                btns.delegate('#solution', 'click', function() {
                    puzInit.solutionPuzzle()
                });

                //Reiniciar puzzle
                btns.delegate('#reset', 'click', function() {
                    puzInit.resetPuzzle()
                });
            ...
            buildEntries: function() {
                ...
                for (var x = 1, p = entryCount; x <= p; ++x) {
                    ...
                    for (var i = 0; i < entries[x - 1].length; ++i) {
                        ...
                        if ($(light).empty() && solution == false) {
                            $(light)
                                .addClass('entry-' + (hasOffset ? x - positionOffset : x) + ' position-' + (x - 1))
                                .append('<input id="fieldCross" maxlength="1" value="" type="text" tabindex="-1" />');

                        } else {
                            $(light)
                                .addClass('entry-' + (hasOffset ? x - positionOffset : x) + ' position-' + (x - 1))
                                .append('<input id="fieldCross" maxlength="1" value="' + letters[i] + '" type="text" tabindex="-1" />');
            ...
            */
            ...
            checkAnswer: function(e) {
                ...
                //console.log('respuesta: ' + puzz.data[activePosition].answer.toLowerCase())
                ...
                if (valToCheck === currVal) {
                    ...
                    if (solved.length == 16) {
                        var audio2 = new Audio('assets/audios/aplausos.mp3');
                        audio2.volume = 0.5;
                        audio2.play();
                        // $('#gameModal').modal('toggle')
                        // $(":input").val('');
                        // $("#across li").removeClass('clue-done');
                        // $("#down li").removeClass('clue-done');
                        // solved = [];

                    } else {
                        var audio = new Audio('assets/audios/correct.wav');
                        audio.volume = 0.5;
                        audio.play();
                    }
                    return;
